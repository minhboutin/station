...
<div class="row">
	<div class="col-1 section white">
		...
		<?php if ($page->startdate()->isEmpty() == FALSE): ?><div class="page date border-bottom medium bold cap">
					<p><?php $day = $page->startdate()->toDate('d');
							 $month = $page->startdate()->toDate('m');
							 $year = $page->startdate()->toDate('y');
						?>
						Publié le <?= $day ?>.<?= $month ?>.<?= $year ?> <?php if ($page->mainauteur()->isEmpty() == FALSE) { echo '— Par ', $page->mainauteur() ; } ?>
					</p>
		</div><?php endif ?>
		<?php if ($page->mainthemes()->isEmpty() == FALSE || $page->maintags()->isEmpty() == FALSE ): ?><div class="page tags border-bottom small cap">
			<ul><?php if ($page->mainthemes()->isEmpty() == FALSE):
					  $themes = explode(',', $page->mainthemes());
					  foreach ($themes as $theme): ?>
				<a class="labels" href="<?= $site->find('stationgdm')->url() ?>/tag:<?= $theme ?>">
					<li class="labels">
						<?= $theme ?>
					</li>
				...
				<div class="row list block border-bottom">
					<div class="col-3-2 row">
						...
						<div class="list content">
							...
							<?php if($content->maindate()->isEmpty() == FALSE): ?><div class="list date border-bottom medium bold cap">
								<p><?= $content->maindate() ?></p>
							</div><?php endif ?>
					...
					if($contents->feedautoselection() == 'none'){
	  			    ...
	  			    foreach($feed_contents as $feed_content): ?><div class="feed col-4 white shadow-bottom-right">
						<a class="card cell linkable" href="<?= $feed_content->url() ?>">
							...
							<?php if ($contents->feedlabels()->toBool() == TRUE || $contents->feedtags()->toBool() == TRUE): ?><div class="card tags small cap">
